...
{
    [EnableCors("*", "*", "*")]
    [RoutePrefix("api/DashBoardSix")]


    [Authorize]
    public class DashBoardSixController : ApiController
    {

        private DashBoardSixDataAccess _context = new DashBoardSixDataAccess(Usuario.Email);
        ...
        [HttpPost]
        [Route("CarregarGraficoBVCTop10Marcas")]
        public HttpResponseMessage GraficoBVCTop10(FiltroPadrao filtro)
        {
            var response = new Response();
            try
            {
                var list = _context.CarregarGraficoBVCTop10Marcas(filtro);
        ...
        [HttpPost]
        [Route("CarregarGraficoBVCEvolutivo")]
        public HttpResponseMessage CarregarGraficoBVCEvolutivo(FiltroPadrao filtro)
        {
            var response = new Response();
            try
            {
                var list = _context.CarregarGraficoBVCEvolutivo(filtro);
