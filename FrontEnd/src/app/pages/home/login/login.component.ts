...
export class LoginComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.logService.getIPAddress().then(s => { this.logService.ipAddress = s?.ip ?? "" });
    
    debugger
    this.formActiveLogin = false;
    this.activatedRoute.queryParams.subscribe(params => {
      var crypt = params['crypt'];
      debugger
      if (crypt) {
        debugger
        var codUser = this.criptografiaService.execDecrypt(crypt);
        if (codUser) {
          debugger
          this.loginIntegrado(parseInt(codUser));
        }
        else {
          this.formActiveLogin = true;
          console.log("Codigo user não encontrado redirecionamento login")
        }
  ...
  login() {
    ...
    this.usuarioInvalido = false;
    this.loginService.login(this.form.login.value, this.form.senha.value,this.logService.ipAddress).subscribe(usuarioModel => {
  ...
  loginIntegrado(CodUser: number) {
    ...
    this.loginService.loginIntegrado(CodUser).subscribe(usuarioModel => {
      debugger
      if (usuarioModel && usuarioModel.CodUser > 0) {
        var user = new LoginAcesso();
        user.EmailUser = usuarioModel.Email;
        user.Senha = usuarioModel.Password;
        this.loginService.token(user).subscribe((dataToken: Token) => {
          debugger
          this.authService.setToken(dataToken.access_token);
          this.session.createSession(usuarioModel);
